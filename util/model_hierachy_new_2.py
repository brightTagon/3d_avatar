...
class model_gs_deg_time_view_hierachy_new_triplane(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        ...
        self.grid_xy = torch.nn.parameter.Parameter(data=torch.rand(1,64,gres,gres), requires_grad=True)
        self.grid_xz = torch.nn.parameter.Parameter(data=torch.rand(1,64,gres,gres), requires_grad=True)
        self.grid_yz = torch.nn.parameter.Parameter(data=torch.rand(1,64,gres,gres), requires_grad=True)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sample_combine = F.normalize(sample_combine.squeeze().T, dim=1)
        sampled_positions = sample_combine
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
        ...
        self.grid_xy_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8), requires_grad=True)
        self.grid_xz_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8), requires_grad=True)
        self.grid_yz_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8), requires_grad=True)

        self.grid_xy_16 = torch.nn.parameter.Parameter(data=torch.rand(1,64,16,16), requires_grad=True)
        self.grid_xz_16 = torch.nn.parameter.Parameter(data=torch.rand(1,64,16,16), requires_grad=True)
        self.grid_yz_16 = torch.nn.parameter.Parameter(data=torch.rand(1,64,16,16), requires_grad=True)

        self.grid_xy_32 = torch.nn.parameter.Parameter(data=torch.rand(1,64,32,32), requires_grad=True)
        self.grid_xz_32 = torch.nn.parameter.Parameter(data=torch.rand(1,64,32,32), requires_grad=True)
        self.grid_yz_32 = torch.nn.parameter.Parameter(data=torch.rand(1,64,32,32), requires_grad=True)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sample_xy_8 = torch.nn.functional.grid_sample(self.grid_xy_8, sample_coord[:,:,:,[0,1]])
        sample_xz_8 = torch.nn.functional.grid_sample(self.grid_xz_8, sample_coord[:,:,:,[0,2]])
        sample_yz_8 = torch.nn.functional.grid_sample(self.grid_yz_8, sample_coord[:,:,:,[1,2]])
        sample_combine_8 = sample_xy_8*sample_xz_8*sample_yz_8
        sample_combine_8 = F.normalize(sample_combine_8.squeeze().T, dim=1)

        sample_xy_16 = torch.nn.functional.grid_sample(self.grid_xy_16, sample_coord[:,:,:,[0,1]])
        sample_xz_16 = torch.nn.functional.grid_sample(self.grid_xz_16, sample_coord[:,:,:,[0,2]])
        sample_yz_16 = torch.nn.functional.grid_sample(self.grid_yz_16, sample_coord[:,:,:,[1,2]])
        sample_combine_16 = sample_xy_16*sample_xz_16*sample_yz_16
        sample_combine_16 = F.normalize(sample_combine_16.squeeze().T, dim=1)

        sample_xy_32 = torch.nn.functional.grid_sample(self.grid_xy_32, sample_coord[:,:,:,[0,1]])
        sample_xz_32 = torch.nn.functional.grid_sample(self.grid_xz_32, sample_coord[:,:,:,[0,2]])
        sample_yz_32 = torch.nn.functional.grid_sample(self.grid_yz_32, sample_coord[:,:,:,[1,2]])
        sample_combine_32 = sample_xy_32*sample_xz_32*sample_yz_32
        sample_combine_32 = F.normalize(sample_combine_32.squeeze().T, dim=1)



        sampled_positions = sample_combine_8+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_2(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sampled_positions = sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_3(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        # sampled_lv4 = self.lv4(t_tensor).squeeze()
        #print(sampled_lv1.shape, sampled_lv2.shape, sampled_lv3.shape, sampled_lv4.shape)
        sampled_times = (sampled_lv2).unsqueeze(0).expand((self.num_v,self.lt))
        #(sampled_lv1+sampled_lv2+sampled_lv3+sampled_lv4).unsqueeze(0).expand((self.num_v,self.lt))
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_4(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sampled_positions = sampled_positions_perlatent #sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_tp(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sampled_positions = sample_combine_32 #sampled_positions_perlatent #sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        # self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        sampled_positions = sample_combine_8 #sampled_positions_perlatent #sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level(torch.nn.Module):
    ...
    def forward(self, sample_coord, t, deg, c2f_level):  #t, deg):
        ...
        #     sample_combine_16 = sample_xy_16*sample_xz_16*sample_yz_16
        #     #sample_combine_16 = F.normalize(sample_combine_16.squeeze().T, dim=1)
        #     sampled_positions += sample_combine_16.squeeze().T
        ...
        #     sample_combine_32 = sample_xy_32*sample_xz_32*sample_yz_32
        #     #sample_combine_32 = F.normalize(sample_combine_32.squeeze().T, dim=1)
        #     sampled_positions += sample_combine_32.squeeze().T

        # if c2f_level>=4:
        #     sample_xy_64 = torch.nn.functional.grid_sample(self.grid_xy_64, sample_coord[:,:,:,[0,1]])
        #     sample_xz_64 = torch.nn.functional.grid_sample(self.grid_xz_64, sample_coord[:,:,:,[0,2]])
        #     sample_yz_64 = torch.nn.functional.grid_sample(self.grid_yz_64, sample_coord[:,:,:,[1,2]])
        #     sample_combine_64 = sample_xy_64*sample_xz_64*sample_yz_64
        #     #sample_combine_64 = F.normalize(sample_combine_64.squeeze().T, dim=1)
        #     sampled_positions += sample_combine_64.squeeze().T

        # if c2f_level>=3:
        #     sampled_positions_perlatent = self.pos_embedding(torch.arange(self.pos_embedding.num_embeddings).to('cuda:0'))
        #     sampled_positions += sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
        ...
        self.grid_xy = torch.nn.parameter.Parameter(data=torch.rand(1,64,64,64), requires_grad=True)
        self.grid_xz = torch.nn.parameter.Parameter(data=torch.rand(1,64,64,64), requires_grad=True)
        self.grid_yz = torch.nn.parameter.Parameter(data=torch.rand(1,64,64,64), requires_grad=True)
        ...
        self.lv1_viewdep = nn.ModuleList([time_latent_module(t//8,lt) for i in range(deg_s)]) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t//4), requires_grad=True)
        self.lv2_viewdep = nn.ModuleList([time_latent_module(t//4,lt) for i in range(deg_s)]) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t//3), requires_grad=True)
        self.lv3_viewdep = nn.ModuleList([time_latent_module(t//2,lt) for i in range(deg_s)]) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t//2), requires_grad=True)
        self.lv4_viewdep = nn.ModuleList([time_latent_module(t,lt) for i in range(deg_s)]) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t), requires_grad=True)
    ...
    def forward(self, sample_coord, t, deg, c2f_level):  #t, deg):
        ...
        # print(sampled_positions.shape)
        t_tensor = torch.Tensor([t]).to('cuda:0')
        sampled_lv1_viewdep = self.lv1_viewdep[deg](t_tensor).squeeze()
        sampled_lv2_viewdep = self.lv2_viewdep[deg](t_tensor).squeeze()
        sampled_lv3_viewdep = self.lv3_viewdep[deg](t_tensor).squeeze()
        sampled_lv4_viewdep = self.lv4_viewdep[deg](t_tensor).squeeze()
        ...
        #(sampled_lv1+sampled_lv2+sampled_lv3+sampled_lv4).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_times = (sampled_lv1+sampled_lv2+sampled_lv3+sampled_lv4).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_times = torch.nn.functional.normalize(sampled_times)
        sampled_view_times = (sampled_lv1_viewdep+sampled_lv2_viewdep+sampled_lv3_viewdep+sampled_lv4_viewdep).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_view_times = torch.nn.functional.normalize(sampled_view_times)

        sampled_times = sampled_times+sampled_view_times
        #sampled_degs = F.tanh(self.deg_embedding(deg).squeeze())

        #print(sampled_positions.shape, sampled_times.shape, sampled_degs.shape)
        #print(sampled_positions.shape, sampled_times.shape)
        x_time = torch.cat([sampled_positions,sampled_times+sampled_view_times], 1)
        #x_view = torch.cat([sampled_positions,sampled_times,sampled_degs], 1)
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon_fix(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
        ...
        self.time_modules = torch.nn.ModuleList([time_latent_hierachy_module(t, lt,max_norm=None) for i in range(deg_s)])
    ...
    def forward(self, sample_coord, t, deg, c2f_level):  #t, deg):
        ...
        sampled_tt = torch.nn.functional.normalize(sampled_time,dim=0).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_view = torch.nn.functional.normalize(sampled_time + sampled_times,dim=0).unsqueeze(0).expand((self.num_v,self.lt))
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon_fix_vary_triplane(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64,k=16):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
        self.grid_xy = torch.nn.parameter.Parameter(data=torch.rand(1,64,gres,gres), requires_grad=True)
        ...
        self.grid_coeff = torch.nn.parameter.Parameter(data=torch.rand(t,deg_s,k)*0.01, requires_grad=True)
        self.grid_xy_basis = torch.nn.parameter.Parameter(data=torch.rand(k,64,gres,gres)*0.01, requires_grad=True)
        self.grid_xz_basis = torch.nn.parameter.Parameter(data=torch.rand(k,64,gres,gres)*0.01, requires_grad=True)
        self.grid_yz_basis = torch.nn.parameter.Parameter(data=torch.rand(k,64,gres,gres)*0.01, requires_grad=True)
    ...
    def forward(self, sample_coord, t, deg, c2f_level,t_index):  #t, deg):
        
        #sampled_positions = F.tanh(self.pos_embedding(torch.arange(50538).to('cuda:0')))
        # sampled_positions_perlatent = self.pos_embedding(torch.arange(self.pos_embedding.num_embeddings).to('cuda:0'))
        coeff = self.grid_coeff[t_index][deg]
        S_xy = torch.nn.functional.grid_sample(torch.einsum('bi,ijkl->bjkl', coeff.unsqueeze(0),self.grid_xy_basis),sample_coord[:,:,:,[0,1]])
        S_xz = torch.nn.functional.grid_sample(torch.einsum('bi,ijkl->bjkl', coeff.unsqueeze(0),self.grid_xz_basis),sample_coord[:,:,:,[0,2]])
        S_yz = torch.nn.functional.grid_sample(torch.einsum('bi,ijkl->bjkl', coeff.unsqueeze(0),self.grid_yz_basis),sample_coord[:,:,:,[1,2]])
        S_combine = S_xy*S_xz*S_yz
        ...
        #sample_combine_8 = F.normalize(sample_combine_8.squeeze().T, dim=1)
        #print(sample_combine_8.shape)
        sampled_aux = S_combine.squeeze().T
        sampled_positions = torch.nn.functional.normalize(torch.nn.functional.normalize(sample_combine.squeeze().T) + sampled_aux) #sampled_positions_perlatent #sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
    ...
    def upsample_latent(self):
        ...
        self.grid_xz = torch.nn.parameter.Parameter(data=F.interpolate(self.grid_xz,scale_factor=2, mode='bilinear',align_corners=False),requires_grad=True)
        self.grid_yz = torch.nn.parameter.Parameter(data=F.interpolate(self.grid_yz,scale_factor=2, mode='bilinear',align_corners=False),requires_grad=True)
        self.grid_xy_basis = torch.nn.parameter.Parameter(data=F.interpolate(self.grid_xy_basis,scale_factor=2, mode='bilinear',align_corners=False),requires_grad=True)
        self.grid_xz_basis = torch.nn.parameter.Parameter(data=F.interpolate(self.grid_xz_basis,scale_factor=2, mode='bilinear',align_corners=False),requires_grad=True)
        self.grid_yz_basis = torch.nn.parameter.Parameter(data=F.interpolate(self.grid_yz_basis,scale_factor=2, mode='bilinear',align_corners=False),requires_grad=True)


class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon_fix_linearbasis(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
        ...
        self.time_module = time_latent_hierachy_module(t, lt,max_norm=None)
        self.time_modules = torch.nn.ModuleList([time_latent_hierachy_module(t, 128,max_norm=None) for i in range(deg_s)])

        self.basis = nn.Linear(128,lt,bias=False)
        self.num_v = num_v
        self.lt = lt
        self.deg_embedding = nn.Embedding(deg_s, ltt, max_norm=1.0)
    ...
    def forward(self, sample_coord, t, deg, c2f_level):  #t, deg):
        ...
        t_tensor = torch.Tensor([t]).to('cuda:0')
        sampled_time = self.time_module(t_tensor)
        sampled_times = self.basis(self.time_modules[deg](t_tensor))
        sampled_tt = torch.nn.functional.normalize(sampled_time,dim=0).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_view = torch.nn.functional.normalize(sampled_time + sampled_times,dim=0).unsqueeze(0).expand((self.num_v,self.lt))
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon_fix_ablation_normalization(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_incon_old_latent(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_full(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)

        self.grid_xy_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8)*0.01, requires_grad=True)
        self.grid_xz_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8)*0.01, requires_grad=True)
        self.grid_yz_8 = torch.nn.parameter.Parameter(data=torch.rand(1,64,8,8)*0.01, requires_grad=True)
    ...
    def forward(self, sample_coord, t, deg, c2f_level):  #t, deg):
        ...
        sample_xy = torch.nn.functional.grid_sample(self.grid_xy_8, sample_coord[:,:,:,[0,1]])
        sample_xz = torch.nn.functional.grid_sample(self.grid_xz_8, sample_coord[:,:,:,[0,2]])
        sample_yz = torch.nn.functional.grid_sample(self.grid_yz_8, sample_coord[:,:,:,[1,2]])
        sample_combine = sample_xy*sample_xz*sample_yz
        sampled_positions = sample_combine.squeeze().T #sampled_positions_perlatent #sample_combine_32 #+sample_combine_16+sample_combine_32+sampled_positions_perlatent
...
class model_gs_deg_time_view_hierachy_new_triplane_h_pos_lowres_lowtime_triplane_level_viewdep(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,gres=64):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=None) 
        nn.init.zeros_(self.pos_embedding.weight)
    ...
    def forward(self, sample_coord, t, deg):  #t, deg):
        ...
        t_tensor = torch.Tensor([t]).to('cuda:0')
        # print("!!!!")
        # print(self.time_module(t_tensor).shape)
        # print(self.time_modules(t_tensor).shape)

        sampled_time_viewdep = self.time_modules[deg](t_tensor).unsqueeze(0) #self.time_modules[deg](t_tensor)
        sampled_time_only = F.normalize(self.time_module(t_tensor).unsqueeze(0)) #F.tanh(sampled_times).unsqueeze(0).expand((self.num_v,self.lt))
        sampled_times = F.normalize(sampled_time_only+sampled_time_only)
        sampled_times = sampled_times.expand((self.num_v,self.lt))
        #sampled_degs = F.tanh(self.deg_embedding(deg).squeeze())
...
class model_gs_deg_time_view_hierachy_new_viewdep_share_time_geo_regularize_gs_upsample(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,viewdep_size=16):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        #self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
        # self.lv1 = torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t//8), requires_grad=True)
        ...
        # self.lv4 = time_latent_module(t,lt) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t), requires_grad=True)
        self.time_module = time_latent_hierachy_module(t, lt)
        self.time_modules = torch.nn.ModuleList([time_latent_hierachy_module(t, viewdep_size, None) for i in range(deg_s)])
        self.viewdep_proj = nn.Linear(viewdep_size, lt,bias=False)
        self.num_v = num_v
        self.lt = lt
        self.viewdep_size = viewdep_size
        self.deg_embedding = nn.Embedding(deg_s, ltt, max_norm=1.0)
    ...
    def forward(self, t, deg):  #t, deg):
        ...
        sampled_times = F.normalize(torch.repeat_interleave(self.time_modules[deg](t_tensor), (self.lt)//(self.viewdep_size), dim=0),dim=0)
...
class model_gs_deg_time_view_hierachy_new_viewdep_share_time_geo_regularize_gs(torch.nn.Module):
    def __init__(self,sz=128,lt=256,ltt=256,per_gs_lt=64,d=256,t=297,deg_s=30,num_v=50538,viewdep_size=16):
        super().__init__()
        self.pos_embedding = nn.Embedding(num_v, per_gs_lt, max_norm=1.0) 
        #self.time_embedding = nn.Embedding(t, lt, max_norm=1.0)
        ...
        # self.lv4 = time_latent_module(t,lt) #torch.nn.parameter.Parameter(data=torch.rand(1,lt,1,t), requires_grad=True)
        self.time_module = time_latent_hierachy_module(t, lt)
        self.time_modules = torch.nn.ModuleList([time_latent_hierachy_module(t, viewdep_size) for i in range(deg_s)])
        self.viewdep_proj = nn.Linear(viewdep_size, lt,bias=False)
        self.num_v = num_v
        self.lt = lt
        self.deg_embedding = nn.Embedding(deg_s, ltt, max_norm=1.0)
    ...
    def forward(self, t, deg):  #t, deg):
        ...
        t_tensor = torch.Tensor([t]).to('cuda:0')
        sampled_time = self.time_module(t_tensor)
        sampled_times = F.normalize(self.viewdep_proj(self.time_modules[deg](t_tensor)), dim=0)
